      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        >
          <div className="flex items-center gap-2 mb-3">
            <div
              className={`w-8 h-8 rounded-xl flex items-center justify-center ${overdueLoans.length > 0 ? "bg-red-100" : "bg-gray-100"}`}
            >
              <Receipt
                size={15}
                className={
                  overdueLoans.length > 0 ? "text-red-500" : "text-gray-400"
                }
              />
            </div>
            <p
              className={`text-[11px] font-semibold uppercase tracking-widest ${overdueLoans.length > 0 ? "text-red-500" : "text-gray-400"}`}
            >
              ادھار میں تاخیر والے افراد کی تعداد
            </p>
          </div>
          <p
            className={`text-4xl font-black leading-none ${overdueLoans.length > 0 ? "text-red-600" : "text-gray-200"}`}
          >
            {overdueLoans.length}
          </p>
        </div>
        ...
        >
          <div className="flex items-center gap-2 mb-3">
            <div
              className={`w-8 h-8 rounded-xl flex items-center justify-center ${lowStockItems.length > 0 ? "bg-orange-100" : "bg-gray-100"}`}
            >
              <Package
                size={15}
                className={
                  lowStockItems.length > 0 ? "text-orange-500" : "text-gray-400"
                }
              />
            </div>
            <p
              className={`text-[11px] font-semibold uppercase tracking-widest ${lowStockItems.length > 0 ? "text-orange-500" : "text-gray-400"}`}
            >
              کم اسٹاک
            </p>
          </div>
          <p
            className={`text-4xl font-black leading-none ${lowStockItems.length > 0 ? "text-orange-600" : "text-gray-200"}`}
          >
      ...
      {allClear ? (
        <div className="bg-white rounded-2xl p-10 text-center ring-1 ring-black/[0.06] shadow-sm">
          ...
          <p className="text-gray-400 mt-1.5 text-sm">
            آج کوئی مسئلہ نہیں۔ دکان چل رہی ہے۔
          </p>
        </div>
      ) : (
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-red-500 text-xs mt-0.5">
                          {diff} {diff === 1 ? "دن گزر گیا" : "دن گزر گئے"}
                        </p>
                      </div>
          ...
          {lowStockItems.length > 0 && (
            <section className="bg-white rounded-2xl ring-1 ring-black/[0.06] shadow-sm overflow-hidden">
              <div className="flex items-center gap-2.5 px-5 py-4 border-b border-orange-100 bg-orange-50/60">
                ...
                <a
                  href="https://www.tajir.app/buyers"
                  target="_blank"
                  rel="noopener noreferrer"
                  onClick={(e) => e.stopPropagation()}
                  className="mr-auto flex items-center gap-1 bg-[#1a7a4a] text-white text-xs font-bold px-2.5 py-1.5 rounded-xl hover:bg-[#155f3a] active:scale-90 transition-all duration-150 shrink-0"
                >
