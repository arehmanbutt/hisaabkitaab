...
import { Plus, MessageCircle, CheckCircle2, ChevronDown, ChevronUp, AlertCircle, Clock, Trash2, FileText } from 'lucide-react';
...
function LoanCard({ loan, onPaid, onDelete }: { loan: Loan; onPaid: (id: string) => void; onDelete: (id: string) => void }) {
    ...
    <>
      <div className={`bg-white rounded-2xl p-4 lg:p-5 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md ${
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <h3 className="font-bold text-gray-800 text-base lg:text-lg">{loan.customerName}</h3>
        ...
        <div className="flex gap-2 mt-4">
          ...
          >
            <CheckCircle2 size={14} />ادا ہو گیا
          </button>
          <button
      ...
      {confirmPaid && (
        <ConfirmDialog
          ...
          cancelText="نہیں"
          onConfirm={() => { onPaid(loan.id); setConfirmPaid(false); }}
      ...
      {confirmDelete && (
        <ConfirmDialog
          ...
          cancelText="منسوخ"
          dangerous
          onConfirm={() => { onDelete(loan.id); setConfirmDelete(false); }}
...
export default function LoansTab() {
  ...
  useEffect(() => { setLoans(storage.getLoans()); }, []);

  const save = (updated: Loan[]) => { setLoans(updated); storage.saveLoans(updated); };

  const addLoan = (loan: Loan) => { save([loan, ...loans]); toast('قرض شامل کر دیا گیا ✓'); };
  const markPaid = (id: string) => { save(loans.map(l => l.id === id ? { ...l, settled: true, settledAt: new Date().toISOString() } : l)); toast('ادائیگی ہو گئی ✓'); };
  const deleteLoan = (id: string) => { save(loans.filter(l => l.id !== id)); toast('ریکارڈ حذف ہو گیا', 'error'); };

  const active = loans
    .filter(l => !l.settled)
    .sort((a, b) => {
      const da = getDaysDiff(a.reminderDate), db = getDaysDiff(b.reminderDate);
  ...
  const overdueCount = active.filter(l => getDaysDiff(l.reminderDate) < 0).length;
    ...
    <div className="px-4 sm:px-6 lg:px-8 xl:px-10 py-6 max-w-7xl mx-auto">
      ...
      {active.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-2xl ring-1 ring-black/[0.06] shadow-sm">
          ...
          <p className="text-gray-400 mt-1.5 text-sm">نیا ادھار شامل کرنے کے لیے بٹن دبائیں</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
          {active.map(loan => (
            <LoanCard key={loan.id} loan={loan} onPaid={markPaid} onDelete={deleteLoan} />
          ))}
        </div>
      )}
        ...
        <div className="mt-6">
          ...
          {showSettled && (
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-3 mt-3">
              {settled.map(loan => (
                <div key={loan.id} className="bg-white rounded-2xl p-4 ring-1 ring-black/[0.04] opacity-50">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="font-semibold text-gray-700 text-sm">{loan.customerName}</p>
                      <p className="text-gray-400 text-xs mt-0.5">{loan.phone}</p>
                    </div>
                    <div className="text-left">
                      <p className="font-bold text-gray-400 line-through text-sm">₨{fmt(loan.amount)}</p>
                      <span className="text-[#1a7a4a] text-xs font-bold">ادا ✓</span>
                    </div>
                  </div>
                </div>
              ))}
      ...
      {showModal && <AddLoanModal onClose={() => setShowModal(false)} onAdd={addLoan} />}
    </div>
  );
}
