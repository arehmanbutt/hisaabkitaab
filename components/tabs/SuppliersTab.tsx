          ...
          <div className="flex-1 min-w-0">
            ...
            {supplier.category && (
              <div className="flex items-center gap-1.5 mt-1.5">
                ...
                <span className="text-[#1a7a4a] text-xs font-bold bg-[#eef7f2] px-2.5 py-0.5 rounded-full">
                  {supplier.category}
                </span>
              </div>
            )}
...
export default function SuppliersTab() {
  ...
  const save = (data: Supplier[]) => {
    setSuppliers(data);
    storage.saveSuppliers(data);
  };
  const addSupplier = (s: Supplier) => {
    save([...suppliers, s]);
    toast("سپلائر شامل کر دیا گیا ✓");
  };
  const remove = (id: string) => {
    save(suppliers.filter((s) => s.id !== id));
    toast("سپلائر حذف ہو گیا", "error");
  };
