...
import { Plus, Minus, AlertTriangle, ExternalLink, Trash2 } from "lucide-react";
        ...
        <div className="flex items-start justify-between gap-2 mb-4">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-gray-400 text-[11px] mt-0.5">
              اکائی: {item.unit} · حد: {item.lowStockThreshold}
            </p>
          </div>
          <button
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="text-center min-w-[4rem]">
              <p
                className={`text-3xl font-black leading-none ${isLow ? "text-red-600" : "text-gray-800"}`}
              >
                {item.quantity}
              </p>
          ...
          {!isLow && (
            <span className="text-[#1a7a4a] text-xs font-semibold bg-[#eef7f2] px-2.5 py-1 rounded-full">
              موجود ✓
            </span>
          )}
        </div>
...
export default function StockTab() {
  ...
  useEffect(() => {
    setItems(storage.getStock());
  }, []);

  const save = (data: StockItem[]) => {
    setItems(data);
    storage.saveStock(data);
  };
  const addItem = (item: StockItem) => {
    save([...items, item]);
    toast("آئٹم شامل کر دیا گیا ✓");
  };
  const remove = (id: string) => {
    save(items.filter((i) => i.id !== id));
    toast("آئٹم حذف ہو گیا", "error");
  };
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 xl:px-10 py-6 max-w-7xl mx-auto">
        ...
        <div>
          ...
          {lowCount > 0 && (
            <div className="flex items-center gap-3">
              ...
              <a
                href="https://tajir.app"
                target="_blank"
                rel="noopener noreferrer"
                className="mt-1 flex items-center gap-1 bg-[#1a7a4a] text-white text-[11px] font-bold px-2 py-1 rounded-xl hover:bg-[#155f3a] active:scale-95 transition-all duration-150"
              >
      ...
      {items.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-2xl ring-1 ring-black/[0.06] shadow-sm">
          ...
          <p className="text-gray-400 mt-1.5 text-sm">
            پہلا آئٹم شامل کریں اور اپنا اسٹاک ٹریک کریں
          </p>
        </div>
      ) : (
      ...
      {showModal && (
        <AddStockModal onClose={() => setShowModal(false)} onAdd={addItem} />
      )}
    </div>
  );
}
